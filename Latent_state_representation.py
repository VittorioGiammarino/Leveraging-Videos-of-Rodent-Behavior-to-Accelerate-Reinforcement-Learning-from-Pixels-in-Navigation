...
from models.LSUR_model import minigridDisentangledVAE, CarlaDisentangledVAE, DisentangledVAE
...
def main():
    ...
    data_set_modified = pickle.load(a_file)
    modified_observations = data_set_modified['observations'].transpose(0,3,1,2)
    
    # a_file = open("offline_data_set/data_set_modified_MiniGrid-Empty-16x16-v0_random.pkl", "rb")
    # data_set_modified = pickle.load(a_file)
    # modified_observations = data_set_modified['observations'].transpose(0,3,1,2)
    ...
    # data_set_real = pickle.load(a_file)
    # real_observations = data_set_real['observations'].transpose(0,3,1,2)
    
    a_file = open("offline_data_set/data_set_MiniGrid-Empty-16x16-v0_random.pkl", "rb")
    data_set_real = pickle.load(a_file)
    real_observations = data_set_real['observations'].transpose(0,3,1,2)
...
a_file = open("offline_data_set/data_set_MiniGrid-Empty-16x16-v0_human_expert.pkl", "rb")
data_set_real = pickle.load(a_file)
real_observations_test = data_set_real['observations'].transpose(0,3,1,2)

a_file = open("offline_data_set/data_set_modified_MiniGrid-Empty-16x16-v0_human_expert.pkl", "rb")
data_set_modified = pickle.load(a_file)
modified_observations = data_set_modified['observations'].transpose(0,3,1,2)

model = Model(class_latent_size = 8, content_latent_size = 64)
model.load_state_dict(torch.load("checkpoints/model.pt"))

# %%
test_1 = torch.FloatTensor(real_observations_test[1]).unsqueeze(0)
test_2 = torch.FloatTensor(modified_observations[100]).unsqueeze(0)

feature_test_1 = model.encoder(test_1/255)
feature_test_2 = model.encoder(test_2/255)

print(feature_test_1)
print(feature_test_2)