...
class GePPO:
    def __init__(self, state_dim, action_dim, action_space_cardinality, max_action, min_action, Entropy = False,
                 num_steps_per_rollout=5000, Load_encoder=True, l_rate_actor=1e-4, gae_gamma = 0.99,  
        ...
        if np.isinf(action_space_cardinality):
            ...
            self.encoder_on = Encoder_Sawyer(state_dim, action_dim)
            ...
            self.encoder_on = Encoder(state_dim, action_space_cardinality)
    ...
    def Generate_and_store_rollout(self, env, args):
        ...
        with torch.no_grad():
            while step < self.num_steps_per_rollout: 
                ...
                if self.action_space == "Discrete":
                    _, old_log_prob_rollout = self.actor.sample_log(embedding, episode_actions)
                    old_log_prob_rollout = old_log_prob_rollout.data.cpu().detach()
                elif self.action_space == "Continuous": 
                    old_log_prob_rollout = self.actor.sample_log(embedding, episode_actions)
        ...
        with torch.no_grad():
            ...
            for i in range(stored_policies):
                ...
                for l in range(len(episode_length)):     
                    ...
                    current_values = self.actor.value_net(embedding).detach().cpu()
                    next_values = torch.cat((self.actor.value_net(embedding)[1:].cpu(), torch.FloatTensor([[0.]]))).detach()
                    episode_deltas = episode_rewards.unsqueeze(-1) + self.gae_gamma*next_values - current_values  
                    
                    if self.action_space == "Discrete":
                        _, log_prob_rollout_full = self.actor.sample_log(embedding, episode_actions)
                        log_prob_rollout_full = log_prob_rollout_full.data.cpu().detach()
                    elif self.action_space == "Continuous": 
                        log_prob_rollout_full = self.actor.sample_log(embedding, episode_actions)
    ...
    def train(self):
        ...
        for k in range(stored_policies):
            ...
            with torch.no_grad():
                ...
                if self.action_space == "Discrete":
                    log_prob, log_prob_rollout = self.actor.sample_log(rollout_embedding, rollout_actions)
    
                elif self.action_space == "Continuous": 
                    log_prob_rollout = self.actor.sample_log(rollout_embedding, rollout_actions)
        ...
        for s in range(max_steps):
            ...
            batch_states = rollout_embedding[minibatch_indices].to(device)
