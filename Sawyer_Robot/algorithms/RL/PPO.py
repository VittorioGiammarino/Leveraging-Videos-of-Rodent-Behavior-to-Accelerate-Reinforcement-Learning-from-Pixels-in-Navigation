...
class PPO:
    def __init__(self, state_dim, action_dim, action_space_cardinality, max_action, min_action, Entropy = False,   
                 num_steps_per_rollout=2000, Load_encoder=True, l_rate_actor=3e-4, gae_gamma = 0.99, gae_lambda = 0.99, 
    ...
    def train(self):
        ...
        with torch.no_grad():
            if self.action_space == "Discrete":
                _, rollout_old_log_pi = self.actor.sample_log(rollout_embedding, rollout_actions)

            elif self.action_space == "Continuous": 
                rollout_old_log_pi = self.actor.sample_log(rollout_embedding, rollout_actions)
