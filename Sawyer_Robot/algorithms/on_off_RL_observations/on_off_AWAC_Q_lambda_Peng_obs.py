...
class on_off_AWAC_Q_lambda_Peng_obs:
    def __init__(self, state_dim, action_dim, action_space_cardinality, max_action, min_action, Domain_adaptation = True, Entropy = True,   
                 num_steps_per_rollout=2000, intrinsic_reward = 0.01, number_obs_off_per_traj=100, Load_encoder=True, reward_given = False, 
                 l_rate_actor=3e-4, l_rate_alpha=3e-4, discount=0.99, tau=0.005, beta=3, gae_gamma = 0.99, gae_lambda = 0.99, minibatch_size=64, 
                 num_epochs=10, alpha=0.2, critic_freq=2, adversarial_loss = "wgan"):
    ...
    def Q_lambda_Peng_off(self, off_policy_data_tot, ntrajs):
        states = []
        actions = []
        target_Q = []
        advantage = []
        ...
        with torch.no_grad():
            ...
            for i in range(ntrajs):
                ...
                if self.action_space == "Discrete":
                    ...
                    actions_temp = self.actor.sample_inverse_model(embedding, embedding_next).squeeze()
                    
                if reward_given:
                    rewards_temp = torch.FloatTensor(off_policy_reward[ind[i]:int(ind[i]+self.number_obs_off_per_traj)]).to(device)
    ...
    def train_inverse_models(self):
        ...
        for _ in range(max_steps):
            ...
            if self.action_space == "Discrete":
                ...
                L_ia = F.mse_loss(inverse_model_action, actions_ims)
    ...
    def train(self, states_off, actions_off, target_Q_off, advantage_off):
        ...
        for i in range(max_steps):
            ...
            if self.action_space == "Discrete":
                ...
                #current Q estimates
                current_Q1, current_Q2 = self.actor.critic_net(batch_embedding, batch_actions)
            ...
            if self.Entropy:
                if self.action_space == "Discrete":
                    ...
                    _, log_pi_state, _ = self.actor.sample(batch_embedding)
