    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with torch.no_grad():
            ...
            if self.action_space == "Discrete":
                ...
                log_pi_next_state, _ = self.actor.sample_log(next_state, next_action) #torch.LongTensor(next_action.detach().numpy().flatten()))
        ...
        if self.action_space == "Discrete":
            ...
            action, _ = self.actor.sample(state)
            log_pi_state, _ = self.actor.sample_log(state, action) #torch.LongTensor(action.detach().numpy().flatten()))
            ...
            if self.BC:
                ...
                actor_loss = lmbda*((action_prob*(self.alpha*log_pi_state-minQ)).sum(dim=1)).mean() + F.mse_loss(action_prob, m)
            else:
