...
class on_off_AWAC_obs(object):
    def __init__(self, state_dim, action_dim, action_space_cardinality, max_action, min_action, Domain_adaptation = True, Entropy = False, 
                 intrinsic_reward = 0.01, Load_encoder=True, reward_given = False, Prioritized = False, l_rate_actor=1e-4, l_rate_critic=3e-4, 
                 l_rate_alpha=1e-4, discount=0.99, beta = 3, tau=0.005, alpha=0.2, critic_freq=2, adversarial_loss = "wgan"):
    ...
    def train(self, replay_buffer_online, off_policy_data_tot, batch_size=128):
        ...
        if self.action_space == "Discrete":
            ...
            Q1_old, Q2_old = self.actor.critic_net(embedding, action)
